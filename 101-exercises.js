...
function isPositiveOdd(number){
    return((isPositive(number)) && (isOdd(number)));
}
...
function isVowel(string){
    ...
    return ((["a", "e", "i", "o", "u"].indexOf(string)) !== -1);
}
...
function countVowels(string){
    var string2 = string.match(/[aeiou]/gi);
    return string2 === null ? 0 : count.length(string2)
...
function endsWithVowel(string){
    string = string.indexOf((string.length - 1));
    return (isVowel(string));
}
...
function startsAndEndsWithVowel(string){
    return ((startsWithVowel(string)) && (endsWithVowel(string)));
}
...
function getHighestNumber(sequence) {
    ...
    for (let i = 0; i < sequence.length; i++)
        (sequence[i] >= largest) ? largest = sequence[i] : largest = largest
    return (largest)
}
...
function onlyPositiveNumbers(sequence){
    var positive = [];
    var negative = [];
    for (let i = 0; i < sequence.length; i++)
        (sequence[i] >= 0) ? positive.push(sequence[i]) : negative.push(sequence[i])
    return positive
}
...
function onlyNegativeNumbers(sequence){
    var positive = [];
    var negative = [];
    for (let i = 0; i < sequence.length; i++)
        (sequence[i] >= 0) ? positive.push(sequence[i]) : negative.push(sequence[i])
...
function hasEvens(sequence){
    for (let i = 0; i < sequence.length; i++)
        ((sequence[i] % 2) === 0) ? true :  false;
        return
}
...
function countEvens(sequence){
    ...
    var count_Odd = 0;
    for (let i = 0; i < sequence.length; i++)
        ((sequence[i] % 2) === 0) ? count_Even += 1 : count_Odd += 1
    return count_Even
}
...
function countNegatives(sequence){
    ...
    var count_Neg = 0;
    for (let i = 0; i < sequence.length; i++)
        (sequence[i] < 0) ? count_Neg += 1 : count_Pos += 1;
    return count_Neg;
}
...
function countOdds(sequence){
    ...
    var count_Neg = 0;
    for (let i = 0; i < sequence.length; i++)
        ((sequence[i] >= 0)) ? count_Pos += 1 : count_Neg += 1
    return count_Pos
}
...
function shortestString(sequence){
    var stirng = ""
    for (let i = 0; i < sequence.largest; i++)
        (sequence[i] <= sequence[i+1]) ? (string = (sequence[i])) : string = string
    return string;
}
...
function longestString(sequence){
    var stirng = ""
    for (let i = 0; i < sequence.largest; i++)
        (sequence[i] >= sequence[i+1]) ? (string = (sequence[i])) : string = string
    return string;
...
// Exercise 88
// Write a function named getYearPublished that takes in an object and returns the value behind the "year_published" key.
...
function getNumberOfBooks(object){
...
function highestPriceBook(object){
    var price = 0 ;
    var index = 0 ;
    for (let i = 0 ; i < object.length; i++){
